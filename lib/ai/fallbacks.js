...
export function buildFallbackTitleSummary(input = {}) {
  ...
  const clarifiedDescription = clipText(input.description || baseSummary, 180) || baseSummary;

  return {
    aiTitle,
    summary: ensureSentence(baseSummary),
    clarifiedDescription: ensureSentence(clarifiedDescription),
    fallbackUsed: true
  };
}
...
export function buildFallbackOnboardingAssistant(input = {}) {
  const language = pickLanguage(input.language);

  if (language === "ru") {
    return {
      answer:
        "Начните с вкладки отправки: выберите категорию, добавьте фото/видео, укажите место и отправьте обращение. Затем откройте панель, чтобы увидеть проблему на карте и статус обработки. Если видите похожую проблему рядом, лучше подтвердить существующую запись вместо создания дубликата.",
      suggestedNextStep: "Откройте вкладку Submission и отправьте первую тестовую заявку.",
      fallbackUsed: true
    };
  }
  ...
  if (language === "uz") {
    return {
      answer:
        "Yuborish bo'limidan boshlang: kategoriya tanlang, foto/video qo'shing, joylashuvni kiriting va murojaat yuboring. Keyin panelda muammoni xaritada va status oqimida kuzating. Agar yaqin joyda o'xshash muammo bo'lsa, yangi xabar o'rniga mavjudini tasdiqlang.",
      suggestedNextStep: "Submission bo'limini ochib birinchi test murojaatingizni yuboring.",
      fallbackUsed: true
    };
  }
  ...
  return {
    answer:
      "Start in Submission: choose category, attach media, set location, and submit your report. Then open Dashboard to see it on the public map and track status updates. If a similar nearby issue already exists, confirm that one instead of creating a duplicate.",
    suggestedNextStep: "Open Submission and send your first test report.",
    fallbackUsed: true
  };
}
