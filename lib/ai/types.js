...
export function normalizeTitleSummaryResponse(payload, fallback) {
  return {
    ...
    clarifiedDescription:
      clipText(payload?.clarifiedDescription || fallback.clarifiedDescription || fallback.summary, 180) ||
      fallback.clarifiedDescription ||
      fallback.summary,
    fallbackUsed: false
  };
}
...
export function normalizeOnboardingResponse(payload, fallback) {
  return {
    answer: clipText(payload?.answer || fallback.answer, 320) || fallback.answer,
    suggestedNextStep: clipText(payload?.suggestedNextStep || fallback.suggestedNextStep, 120) || fallback.suggestedNextStep,
    fallbackUsed: false
  };
}
